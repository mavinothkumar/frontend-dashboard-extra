...
if ( defined( 'BC_FED_EXTRA_PLUGIN_VERSION' ) ) {
	add_filter(
		'fed_plugin_versions', function ( $version ) {
			return array_merge( (array) $version, array( 'extra' => 'Extra (' . BC_FED_EXTRA_PLUGIN_VERSION . ')' ) );
		}
	);
}
...
if ( ! function_exists( 'fede_wp_enqueue_scripts' ) ) {
	...
	function fede_wp_enqueue_scripts() {
		if ( defined( 'BC_FED_EXTRA_PLUGIN' ) && defined( 'BC_FED_EXTRA_PLUGIN_VERSION' ) ) {
			wp_enqueue_script(
...
if ( ! function_exists( 'fed_image_mime_types' ) ) {
	...
	function fed_image_mime_types() {
		return array(
			...
			'application/pdf'                                                           => site_url() . '/wp-includes/images/media/document.png',
